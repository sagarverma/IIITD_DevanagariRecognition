...
from utils import EnglishImagePreloader2, text_loader
...
def pred_to_image(outputs):
    outputs = outputs.data.cpu().numpy()
    noutputs = []
    for output in outputs:
        word = ''
        for c in output:
            word += inv_class_map[c]

        image, _ = text_loader(word)
        noutputs.append(image)
    return torch.from_numpy(np.asarray(noutputs))

def train_model(model, criterion, optimizer, num_epochs=2000):
    """
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            for data in dataloaders[phase]:
                ...
                noutputs = pred_to_image(preds)
                if use_gpu:
                    noutputs = noutputs.cuda(DEVICE)
                    
                print (outputs.size(), labels.size(), preds.size(), noutputs.size())
                preds_size = Variable(torch.IntTensor([outputs.size(0)] * batch_size))
                loss = criterion(noutputs, labels)
...
image_datasets = {'train': EnglishImagePreloader2(data_dir + train_csv),
                    'test': EnglishImagePreloader2(data_dir + test_csv)}
...
#Train model
model_conv = train_model(model_conv, criterion, optimizer, num_epochs=num_epochs)
