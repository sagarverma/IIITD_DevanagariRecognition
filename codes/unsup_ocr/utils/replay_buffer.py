...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        act1_batch     = self.action1[idxes]
        act2_batch     = self.action2[idxes]
        rew_batch      = self.reward[idxes]
        next_obs_batch = np.concatenate([self._encode_observation(idx + 1)[np.newaxis, :] for idx in idxes], 0)
        done_mask      = np.array([1.0 if self.done[idx] else 0.0 for idx in idxes], dtype=np.float32)

        return obs_batch, act1_batch, act2_batch, rew_batch, next_obs_batch, done_mask
    ...
    def store_frame(self, frame):
        ...
        if self.obs is None:
            ...
            self.action1  = np.empty([self.size],                     dtype=np.int32)
            self.action2  = np.empty([self.size],                     dtype=np.int32)
    ...
    def store_effect(self, idx, action1, action2, reward, done):
        ...
        """
        self.action1[idx] = action1
        self.action2[idx] = action2
